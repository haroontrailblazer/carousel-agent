...
function CropPreview({ image, crop, background, label }: {
  image: HTMLImageElement; crop: LogoCrop; background: string; label: string
}) {
  const canvas = React.useRef<HTMLCanvasElement>(null)
  React.useEffect(() => {
    if (canvas.current) drawDesignLogo(canvas.current, image, crop, 128)
  }, [image, crop])
...
export function LogoCropDialog({ image, design, onApply, onClose, restoreFocus }: {
  ...
  const paintCanvas = React.useCallback((canvas: HTMLCanvasElement | null) => {
    if (!canvas) return
    try { drawDesignLogo(canvas, image, crop, 512) }
  ...
  function apply() {
    try { onApply(prepareDesignLogo(image, crop)) }
    ...
    <DialogContent className="logo-crop-dialog" onCloseAutoFocus={event => { event.preventDefault(); restoreFocus() }}>
      ...
      <DialogBody className="logo-crop-body space-y-0">
        ...
        <div className="logo-crop-examples">
          <CropPreview image={image} crop={crop} background="#000000" label="Cover" />
          <CropPreview image={image} crop={crop} background={design.inside.background} label="Inside" />
          <CropPreview image={image} crop={crop} background={(design.cta ?? design.inside).background} label="CTA" />
        </div>
