...
| file | must expose |
|---|---|
| app/tools/media_tools.py | `find_source_clip(news: dict) -> dict`, `download_and_trim(url, max_s=8, min_s=4) -> str(path)`, `compose_cover(media_path, title, highlight, is_video: bool) -> dict{video_path, poster_path, duration_s}` — composites `settings.cover_overlay_template` + centered title (white + orange highlight phrase) per skills/cover-style.md, outputs 1080x1350 mp4 (+ poster png). ffmpeg via subprocess (`settings.ffmpeg_bin`), yt-dlp via its python API. |
| app/tools/image_gen.py | `generate_slide_image(template_ref: str, copy_lines: list[str], headline: str, slide_no: int, out_path: str) -> str` — OpenAI Images API, model `settings.image_model` (gpt-image-2), size "1080x1350", template reference image + STRICT verbatim-text prompt. Also `generate_cta_image(...)`. |
...
| app/agents/stitch_verify.py | `build_stitch_verify_agent()` — assembles Bundle (ordered artifacts: cover video first), QA checks (count <= 10, durations 4-8 s, copy-vs-rendered text via LLM vision or size heuristics, line budgets), writes K_BUNDLE + K_QA_REPORT; critical issues → auto-route back (set K_REWORK_PLAN) without mailing. |
