...
| app/agents/phrasing.py | `build_phrasing_agent()` - LlmAgent LiteLlm(settings.phrasing_model), output_schema CopySet, output_key K_COPY; enforces plan style, line budget, and the repository-wide prohibition on em dashes. Audience-facing Pydantic schemas reject the forbidden character before rendering or publishing. |
