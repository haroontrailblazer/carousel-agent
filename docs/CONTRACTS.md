...
| app/agents/template_design.py | `build_template_design_agent()` - LlmAgent + image_gen tool; passes the news item, research brief, and slide purpose into each visual prompt. Only subject-introduction slides receive a sourced cover-media crop for deterministic lower-zone compositing. Saves one PNG per body slide and writes K_BODY_SLIDES. |
